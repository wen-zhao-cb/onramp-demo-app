...
export default function WalletBox() {
  ...
  const setNetworkOption = useCallback((event: ChangeEvent<HTMLSelectElement>) => {
  ...
  const generateOnrampLink = useCallback(async () => {
    ...
    setOnrampLink(`https://pay.coinbase.com/buy/select-asset?appId=6fcd9c6e-e3fc-4bed-823e-3979dc2704d6`+
      `&addresses={"${address}":["${shorten_network}"]}`)
    }, [address, network]);

    const generateOfframpLink = useCallback(async () => {
      let shorten_network = network.split("-")[0]
      setOfframpLink(`https://pay.coinbase.com/v3/offramp/input?appId=6fcd9c6e-e3fc-4bed-823e-3979dc2704d6`+
        `&addresses={"${address}":["${shorten_network}"]}&redirectUrl=https:portfolio.metamask.io`)
      }, [address, network]);

  const wallet_address = useMemo(() => {
    if  (address.length ==0)
      return "Generate or Load a wallet";
    return (
      `${address}`
    );
  }, [address]);
  ...
  const link = useMemo(() => {
    if (address.length ==0 )
      return "Generate or load a wallet first to create your one time URL";
    return (

      `https://pay.coinbase.com/v3/offramp/input?appId=6fcd9c6e-e3fc-4bed-823e-3979dc2704d6`+
      `&addresses={${address}:[${network}]}&redirectUrl=https:portfolio.metamask.io`
    );
  }, [address, network]);
